...
from evaluate.eval_mir import transcription_metrics, multipitch_metrics
...
def evaluate(model, dataloader, device, \
    ...
    for i, (x, y, audio) in enumerate(dataloader):
        ...
        pitch_ground_truth.append(y.detach().cpu().numpy())
        pitch_predictions.append((torch.sigmoid(preds).detach().cpu().numpy() > threshold) * 1)
    ...
    # Average the metrics across the batch
    for key in metrics_frames:
        metrics_frames[key] = np.mean(metrics_frames[key])
        metrics_frames[key] = np.round(metrics_frames[key], 4)
    for key in metrics_note:
        metrics_note[key] = np.mean(metrics_note[key])
        metrics_note[key] = np.round(metrics_note[key], 4)
...
def main(config):
    ...
    best_model = load_shallow(config["in_features"], config["hidden_units"], \
            config["dropout"])
    checkpoint_path = config["save_dir"] + f"/{model_name}"
