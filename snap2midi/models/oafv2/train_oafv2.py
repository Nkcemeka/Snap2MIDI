...
from .oafv2 import OnsetsAndFramesV2
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        ...
        if dataset.augmentator is not None:
            ...
            if getattr(loader, "persistent_workers", False):
                raise RuntimeError(
                    "oafv2 augmentation needs persistent_workers=False: workers "
...
class OAFV2DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        ...
        # checkpoint selection monitors val_total_loss.
        self.train_dataset = OAFV2Dataset(self.config, [f"{train_path}"],
                                          augmentator=build_augmentator(self.config))

        # We don't necessarily need a validation dataset
        if Path(val_path).exists():
            self.val_dataset = OAFV2Dataset(self.config, [f"{val_path}"])
        else:
...
def main(config):
    ...
    if val_flag:
        checkpoint_callback = ModelCheckpoint(
            ...
            filename='oafv2-{epoch:02d}-{val_total_loss:.2f}',
