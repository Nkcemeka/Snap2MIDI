    ...
    def __init__(self, config: dict, emb_paths: Optional[list[str]]=None,
        ...
        ----
            ...
            augmentator (Augmentator | None):
                ...
                model computes its own mel from that waveform, so nothing else
                has to change; the labels are read from the store and are
                untouched. Leave None for a clean baseline.
        """
        super().__init__()
        ...
        # silently. See EpochUpdateCallback in train_oafv2.py.
        self.epoch = 0
