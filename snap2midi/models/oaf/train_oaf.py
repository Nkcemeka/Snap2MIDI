...
from .dataset_oaf import FEATURE_KEYS
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        ...
        if dataset.augmentator is not None:
            ...
            if getattr(loader, "persistent_workers", False):
                raise RuntimeError(
                    "oaf augmentation needs persistent_workers=False: workers "
...
class OAFDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        ...
        self.train_dataset = OAFDataset(
            [f"{train_path}"],
            augmentator=build_augmentator(self.config),
            feature_config={k: self.config[k] for k in FEATURE_KEYS if k in self.config})
...
def main(config):
    ...
    if val_flag:
        checkpoint_callback = ModelCheckpoint(
            ...
            filename='oaf-{epoch:02d}-{val_total_loss:.2f}',
