...
def load_conv_shallow(config: dict):
    # Load the necessary components from the config
    path = config["conv_checkpoint"]
    in_features = config["in_features"]
    out_features = config["out_features"]
    model = ConvShallowTranscriber(in_features=in_features, out_features=out_features)
...
def inference(audio_path: str, config: dict, feature_str: str, filename: str):
    ...
    # Get the models
    model = load_conv_shallow(config)
    result: list = []
