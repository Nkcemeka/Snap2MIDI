"""
    ...
    Date: 6/12/2025
    Description: Training script for the conv shallow transcriber!
"""
...
import wandb
import json
from .conv_shallow import ConvShallowTranscriber
from .datasets.dataset_conv_shallow import ConvShallowDataset
...
def main(config):
    # Create datasets 
    train_dataset = ConvShallowDataset(config["paths"], dataset_type="train")
    valid_dataset = ConvShallowDataset(config["paths"], dataset_type="val")
    test_dataset = ConvShallowDataset(config["paths"], dataset_type="test")
    ...
    # Load the model
    model = ConvShallowTranscriber(config["in_features"],\
    ...
    best_model = model = ConvShallowTranscriber(config["in_features"], \
            dropout = config["dropout"], out_features = config["out_features"])
