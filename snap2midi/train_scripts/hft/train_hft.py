"""
    ...
    Date: 7/14/2025
    Description: Training script for HFT model!
"""
# Imports
import torch
...
from .datasets.hft_dataset import HFTDataset
...
def main(train_config: dict):
    ...
    with open(train_config["paths"][1], 'r') as f:
        config = json.load(f)
    
    # Create HFT dataset instances for train, validation, and test sets
    train_dataset = HFTDataset(train_config["n_slice"], train_config["paths"][0], config, split="train")
    valid_dataset = HFTDataset(train_config["n_slice"], train_config["paths"][0], config, split="val")
    test_dataset = HFTDataset(train_config["n_slice"], train_config["paths"][0], config, split="test")

    # Create DataLoaders for each dataset
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=train_config["batch_size"], shuffle=True)
    valid_dataloader = torch.utils.data.DataLoader(valid_dataset, batch_size=train_config["batch_size"], shuffle=False)
    ...
    Path(train_config["save_dir"]).mkdir(exist_ok=True, parents=True)
    last_epoch = max(1, last_epoch)  # We use 1 since we save with epoch + 1
    best_loss = float('inf')
    device = "cuda" if torch.cuda.is_available() else "cpu"

    for epoch in tqdm(range(last_epoch-1, train_config["epochs"]), desc="Training Epochs"):
        train_loss_dict = train_step(model, optimizer, loss_onset1, loss_offset1, loss_frames, loss_velocity1, \
                                     loss_onset2, loss_offset2, loss_frames2, loss_velocity2, \
                                     train_config["weight_A"], train_config["weight_B"], train_dataloader, \
                                     device, train_config["verbose"])
        ...
        if valid_loss_dict['valid_total_loss'] < best_loss:
            ...
            checkpoint_path = Path(train_config["save_dir"]) / f"checkpoint_{epoch + 1}.pt"
            torch.save({
                'epoch': epoch + 1,
    ...
    test_loss_dict = validate_step(best_model, loss_onset1, loss_offset1, loss_frames, loss_velocity1, \
                                  loss_onset2, loss_offset2, loss_frames2, loss_velocity2, \
                                  train_config["weight_A"], train_config["weight_B"], test_dataloader, \
                                  device, train_config["verbose"], prefix="test")
