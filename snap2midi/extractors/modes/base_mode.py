"""
    ...
    Date: 7/12/2025
    Description: Base class for mode extractors in the Snap2MIDI project.
"""
...
class BaseMode:
    ...
    def get_label_roll(self, midi: pretty_midi.PrettyMIDI, \
                  duration: int | float, hop_size: float, idx: int, pr_rate) -> dict:
        """
            ...
            if not instrument.is_drum:
                for note in instrument.notes:
                    ...
                    if end_frame < num_frames:
                        ...
                        (note.end - start) - (end_frame / pr_rate)

                        if start_frame >= 0:
                            label_onsets[start_frame, pitch] = 1
                            label_reg_onsets[start_frame, pitch] = \
                            (note.start - start) - (start_frame / pr_rate)
                        else:
        ...
        for pitch in range(128):
            label_reg_onsets[:, pitch] = self.get_reg(label_reg_onsets[:, pitch], pr_rate)
            label_reg_offsets[:, pitch] = self.get_reg(label_reg_offsets[:, pitch], pr_rate)
        ...
        for instrument in midi.instruments:
            if not instrument.is_drum:
                for cc in [_e for _e in instrument.control_changes
                    ...
                    frame_now = int(round((cc.time - start) * pr_rate))
                    ...
                    if not is_pedal_on and is_current_pedal_on:
                        ...
                        (time_now) - (frame_now / pr_rate)

                        if frame_pedal_on >= 0:
                            pedal_onset[frame_pedal_on] = 1
                            pedal_reg_onset[frame_pedal_on] = \
                            (onset_time) - (frame_pedal_on / pr_rate)
                        
                        is_pedal_on = False
                        pedal_events.append([onset_time, time_now])
        
        # Get the pedal regressed values
        pedal_reg_onset = self.get_reg(pedal_reg_onset, pr_rate)
        pedal_reg_offset = self.get_reg(pedal_reg_offset, pr_rate)
    ...
    def get_reg(self, input: np.ndarray, pr_rate) -> np.ndarray:
        """
        ...
        # onset/offset can occur whithin a frame resolution of 1/pr_rate
